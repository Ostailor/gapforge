...
V22_FIXTURE_NAMES = [
    "complete_pilot_benchmark",
    "underpowered_alpha_overclaim",
    "missing_related_work",
    "missing_required_baseline",
    "synthetic_deployment_overclaim",
    "pilot_manuscript_honest",
    "pilot_reviewer_blockers",
]
V23_FIXTURE_NAMES = [
    "complete_publication_candidate",
    "pilot_workshop_candidate",
    "revise_due_to_related_work",
    "revise_due_to_baselines",
    "no_go_due_to_novelty_duplicate",
...
V25_FIXTURE_NAMES = [
    "vetted_benchmark_primary_fit",
    "vetted_benchmark_auxiliary_fit",
    "vetted_benchmark_no_fit",
    "venue_style_safe_rewrite",
    "copied_prose_blocked",
    "openreview_dataset_fixture",
...
V261_CALIBRATION_FIXTURE_NAMES = [
    ...
    "artifact_complete_but_bad_novelty",
    "strong_related_work_missing_artifact",
    "workshop_candidate_honest",
...
def default_v22_fixture_root() -> Path:
    return Path.cwd() / "tests" / "fixtures" / "selected_benchmark_v22"


def default_v23_fixture_root() -> Path:
...
def default_v25_fixture_root() -> Path:
...
def load_fixture(name: str, root: Path | None = None) -> EvalFixture:
    ...
    if not path.exists():
        ...
        v22_path = default_v22_fixture_root() / name
        if v22_path.exists():
            return load_v22_fixture(name, default_v22_fixture_root())
...
def load_v22_fixture(name: str, root: Path | None = None) -> EvalFixture:
    fixture_root = root or default_v22_fixture_root()
    path = fixture_root / name
    if not path.exists():
        raise FileNotFoundError(f"Unknown v2.2 selected benchmark eval fixture: {name}")
    raw = _read_json(path / "fixture.json")
    topic = str(raw.get("topic", name.replace("_", " ")))
    papers = [
        Paper(
            id=f"paper-{name}",
            title=f"Selected pilot benchmark fixture paper for {topic}",
            authors=["GapForge fixture"],
            abstract="Synthetic offline fixture metadata for v2.2 selected benchmark pilot evaluation.",
            year=2026,
            source="fixture",
        )
    ]
    return EvalFixture(
        ...
        known_good_gaps=[
            Gap(
                id=f"gap-{name}",
                title=topic,
                description="Offline v2.2 pilot benchmark behavior fixture.",
                supporting_paper_ids=[papers[0].id],
                why_existing_work_does_not_solve_it=(
                    "Fixture encodes pilot power, distributions, baselines, analysis, review, manuscript, and release-gate behavior."
                ),
                minimum_experiment_needed="Use fixture selected pilot benchmark records and release-gate expectations.",
                risk_that_gap_is_fake="This is synthetic fixture data and is not a real benchmark result.",
                confidence="medium",
            )
        ],
        ...
        is_v22=True,
        selected_benchmark_fixture=raw,
        selected_benchmark_v22_fixture=raw,
    )


def load_v23_fixture(name: str, root: Path | None = None) -> EvalFixture:
    fixture_root = root or default_v23_fixture_root()
    path = fixture_root / name
    if not path.exists():
        raise FileNotFoundError(f"Unknown v2.3 selected benchmark eval fixture: {name}")
    raw = _read_json(path / "fixture.json")
    topic = str(raw.get("topic", name.replace("_", " ")))
    papers = [
        Paper(
            id=f"paper-{name}",
            title=f"Selected main benchmark fixture paper for {topic}",
            authors=["GapForge fixture"],
            abstract="Synthetic offline fixture metadata for v2.3 selected benchmark main-scale decision evaluation.",
            year=2026,
            source="fixture",
        )
    ]
    return EvalFixture(
        ...
        known_good_gaps=[
            Gap(
                id=f"gap-{name}",
                title=topic,
                description="Offline v2.3 main-scale selected benchmark decision fixture.",
                supporting_paper_ids=[papers[0].id],
                why_existing_work_does_not_solve_it=(
                    "Fixture encodes main power, related-work completion, baselines, analysis, go/no-go, review, "
                    "manuscript maturity, and release-gate honesty."
                ),
                minimum_experiment_needed="Use fixture v2.3 decision records and release-gate expectations.",
                risk_that_gap_is_fake="This is synthetic fixture data and is not a real benchmark result.",
                confidence="medium",
            )
        ],
        ...
        is_v23=True,
        selected_benchmark_fixture=raw,
        selected_benchmark_v23_fixture=raw,
    )
...
def load_v25_fixture(name: str, root: Path | None = None) -> EvalFixture:
    fixture_root = root or default_v25_fixture_root()
    path = fixture_root / name
    if not path.exists():
        raise FileNotFoundError(f"Unknown v2.5 eval fixture: {name}")
    raw = _read_json(path / "fixture.json")
    topic = str(raw.get("topic", name.replace("_", " ")))
    papers = [
        Paper(
            id=f"paper-{name}",
            title=f"v2.5 eval fixture paper for {topic}",
            authors=["GapForge fixture"],
            abstract="Synthetic offline fixture metadata for v2.5 real benchmark grounding and reviewer calibration evaluation.",
            year=2026,
            source="fixture",
        )
    ]
    return EvalFixture(
        ...
        known_good_gaps=[
            Gap(
                id=f"gap-{name}",
                title=topic,
                description="Offline v2.5 selected benchmark grounding and reviewer calibration fixture.",
                supporting_paper_ids=[papers[0].id],
                why_existing_work_does_not_solve_it=(
                    "Fixture encodes vetted benchmark fit, adapter transparency, venue-style safety, "
                    "OpenReview-style review calibration, drastic review, and release-gate behavior."
                ),
                minimum_experiment_needed="Use fixture v2.5 records and release-gate expectations.",
                risk_that_gap_is_fake="This is synthetic fixture data and is not a real benchmark, review dataset, or paper result.",
                confidence="medium",
            )
        ],
        known_bad_gaps=[],
        ...
        is_v25=True,
        selected_benchmark_fixture=raw,
        selected_benchmark_v25_fixture=raw,
    )
...
def load_calibration_fixture(name: str, root: Path | None = None) -> EvalFixture:
    ...
    path = fixture_root / name
    if not path.exists():
        raise FileNotFoundError(f"Unknown v2.6.1 calibration eval fixture: {name}")
    raw = _read_json(path / "eval_input.json")
    topic = str(raw.get("topic", name.replace("_", " ")))
    papers = [
        Paper(
            id=f"paper-{name}",
            title=f"v2.6.1 calibration fixture paper for {topic}",
            authors=["GapForge fixture"],
            abstract="Synthetic offline fixture metadata for v2.6.1 eval calibration behavior.",
            year=2026,
            source="fixture",
        )
    ]
    return EvalFixture(
        ...
        known_good_gaps=[
            Gap(
                id=f"gap-{name}",
                title=topic,
                description="Offline v2.6.1 calibration fixture for blocker-aware grouped eval scoring.",
                supporting_paper_ids=[papers[0].id],
                why_existing_work_does_not_solve_it=(
                    "Fixture encodes workflow completeness, safety blockers, and paper-quality readiness separately."
                ),
                minimum_experiment_needed="Use fixture calibration records and grouped score expectations.",
                risk_that_gap_is_fake="This is synthetic fixture data and is not a real paper-quality judgement.",
                confidence="medium",
            )
        ],
        known_bad_gaps=[],
        ...
        selected_benchmark_fixture=raw,
        selected_benchmark_v26_fixture=raw,
        expected_score_groups=_read_json(path / "expected_score_groups.json"),
        expected_blockers=_read_json(path / "expected_blockers.json"),
        expected_release_behavior=_read_json(path / "expected_release_behavior.json"),
    )


def load_calibration_fixtures(names: list[str] | None = None, root: Path | None = None) -> list[EvalFixture]:
    selected = names or V261_CALIBRATION_FIXTURE_NAMES
    return [load_calibration_fixture(name, root) for name in selected]
...
def load_v22_fixtures(names: list[str] | None = None, root: Path | None = None) -> list[EvalFixture]:
    selected = names or V22_FIXTURE_NAMES
    return [load_v22_fixture(name, root) for name in selected]
